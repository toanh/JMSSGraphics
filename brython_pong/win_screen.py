...
ball_image = jmss.loadImage("ball.png")
...
def Game():
    ...
    #code for the game
        ...
        if paddle1_lives <= 0 or paddle2_lives <= 0:
            ...
            if ball_xpos + 24 >= paddle2_xpos and ball_ypos >= paddle2_ypos and \
               ball_xpos + 24 <= paddle2_xpos + 24 and ball_ypos <= paddle2_ypos + 144:
                # the ball has collided with the left paddle
                ball_xpos = paddle2_xpos - 24   # snap the ball to the right edge of paddle
                                                # so that the ball does not appear 'inside' the paddle
                ball_xdir *= -1                 # abbreviation for ball_xdir = ball_xdir * -1
                jmss.playSound(blip_sound, loop = False)        
