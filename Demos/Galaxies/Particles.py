...
def spawnParticle(t):
    c = (math.sin(t * 0.1) / 2) + 0.50
    # print (c)
    particles.append({'x': 0,
                      'y': 100,
                      'velx': 20 * (random.random() - 0.5),
...
def mainLoop():
    ...
    for particle in particles:
        ...
        particle["r"] = particle["r"] - 0.02
        if (particle["r"] < 0.0):
            particle["r"] = 0.0
        particle["g"] = particle["g"] - 0.02
        if (particle["g"] < 0.0):
            particle["g"] = 0.0
        particle["b"] = particle["b"] - 0.02
        if (particle["b"] < 0.0):
            particle["b"] = 0.0
    ...
    for particle in particles:
        ...
        jmss.drawImage(image, (particle['x'], particle['y']), particle['r'] * 360, None, particle['r'] * 255, (32,32))

    t = t + 1

    jmss.drawImage(image, (-400, -284), t, (100,0))
...
jmss = Graphics(800, 600, "My Window")
jmss.run(MainLoop)
'''
