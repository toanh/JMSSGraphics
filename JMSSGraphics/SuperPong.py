...
class Paddle(Entity):
    ...
    def Update(self):
        ...
        if jmss.isKeyDown(self.fire) and self.timer == 0:
            ...
            if (self.fireDir > 0):
                game.SpawnBullet(self.x + self.gun.width / 2, self.y + self.height / 2, self.fireDir * 20, self)
            else:
                game.SpawnBullet(self.x - self.gun.width / 2, self.y + self.height / 2, self.fireDir * 20, self)
...
class Game:
    ...
    def drawTriangleList(self, triList):
        ...
        for i in range(0, len(triList), 3):
            jmss.drawLine(triList[i][0], triList[i][1], triList[i + 1][0], triList[i + 1][1], r = colour[0], g = colour[1], b = colour[2], width = 3)
            jmss.drawLine(triList[i + 1][0], triList[i + 1][1], triList[i + 2][0], triList[i + 2][1], r = colour[0], g = colour[1], b = colour[2], width = 3)
            jmss.drawLine(triList[i + 2][0], triList[i + 2][1], triList[i][0], triList[i][1], r = colour[0], g = colour[1], b = colour[2], width = 3)
    ...
    def Cleanup(self):
        jmss.pauseSound(self.background_music)
    ...
    def DrawGame(self):
        ...
        jmss.drawImage(self.p1.image, mousePos[0], mousePos[1])
        if jmss.isMousePressed(MOUSE_BUTTON_LEFT):
            mousePos = jmss.getMousePos()
            jmss.drawImage(self.p2.image, mousePos[0], mousePos[1])
        '''
...
jmss = Graphics(width=600, height=400, title="Super Pong", fullscreen = False)
